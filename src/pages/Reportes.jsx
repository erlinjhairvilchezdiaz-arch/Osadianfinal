...
export default function Reportes() {
  ...
  async function cargar() {
    ...
    const agrupado = {};
    items.forEach((i) => {
      if (!agrupado[i.nombre_producto]) agrupado[i.nombre_producto] = { name: i.nombre_producto, unidades: 0, total: 0 };
      agrupado[i.nombre_producto].unidades += i.cantidad;
      agrupado[i.nombre_producto].total += i.cantidad * Number(i.precio_unitario);
    });
      ...
      <main style={{ flex: 1, padding: "32px 40px", maxWidth: 1100 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {periodos.map((p) => (
              <button key={p} onClick={() => setPeriodo(p)} style={{ padding: "8px 14px", borderRadius: 20, border: `1px solid ${colors.border}`, fontSize: 12.5, fontWeight: 500, cursor: "pointer", background: periodo === p ? colors.plum : colors.card, color: periodo === p ? colors.bg : colors.textMuted }}>{p}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 22 }}>
          {[
            { label: "Total vendido", value: loading ? "—" : `S/ ${resumen.totalVentas.toFixed(2)}` },
            { label: "Ganancia neta", value: loading ? "—" : `S/ ${resumen.gananciaNeta.toFixed(2)}`, color: colors.sageText },
            { label: "Margen", value: loading || resumen.totalVentas === 0 ? "—" : `${((resumen.gananciaNeta / resumen.totalVentas) * 100).toFixed(1)}%` },
            { label: "Número de ventas", value: loading ? "—" : resumen.cantidadVentas },
          ].map((c, i) => (
            <div key={i} style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: 10, padding: "16px 20px" }}>
              <p style={{ fontSize: 11.5, letterSpacing: "0.04em", textTransform: "uppercase", color: colors.textSoft, fontWeight: 600, margin: "0 0 6px" }}>{c.label}</p>
              <p style={{ fontFamily: fonts.display, fontSize: 22, fontWeight: 600, margin: 0, color: c.color || colors.plum }}>{c.value}</p>
            </div>
          ))}
        </div>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", gap: 14, height: 140 }}>
              {ventasPorDia.map((v) => (
                <div key={v.dia} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                  ...
                  <div style={{ width: "100%", maxWidth: 44, height: `${Math.max((v.monto / maxVenta) * 100, 4)}px`, background: v.monto === Math.max(...ventasPorDia.map((x) => x.monto)) ? colors.rose : colors.border, borderRadius: "4px 4px 0 0" }} />
